...
class mzruiBlocksManager {
	static #mousedownFns = Object.freeze( {
		move: mzruiBlocksManager.#onmousedownMove,
		cropA: mzruiBlocksManager.#onmousedownCropA,
		cropB: mzruiBlocksManager.#onmousedownCropB,
	} );
	static #mousemoveFns = Object.freeze( {
		crop: mzruiBlocksManager.#onmousemoveCrop,
		move: mzruiBlocksManager.#onmousemoveMove,
		deletion: mzruiBlocksManager.#onmousemoveDeletion,
		selection1: mzruiBlocksManager.#onmousemoveSelection1,
		selection2: mzruiBlocksManager.#onmousemoveSelection2,
	} );
	static #mouseupFns = Object.freeze( {
		moving: mzruiBlocksManager.#onmouseupMoving,
		deleting: mzruiBlocksManager.#onmouseupDeleting,
		"cropping-a": mzruiBlocksManager.#onmouseupCroppingA,
		"cropping-b": mzruiBlocksManager.#onmouseupCroppingB,
		"selecting-1": mzruiBlocksManager.#onmouseupSelecting1,
		"selecting-2": mzruiBlocksManager.#onmouseupSelecting2,
	} );
	rootElement = null;
	timeline = null;
	#data = null;
	...
	constructor( opts ) {
		...
		this.#opts = opts;
		this.#opts.oneditBlock = opts.oneditBlock || MZRUI.$noop;
		this.#blockDOMChange = opts.blockDOMChange;
		this.#elSelection = opts.selectionElement;
		this.#nlRows = opts.rootElement.getElementsByClassName( "mzrui-row" );
	...
	// .........................................................................
	#isBlc( el ) {
		return el.classList.contains( "mzruiBlocksManager-block" );
	}
	#getBlc( el ) {
		if ( this.#isBlc( el ) ) {
			return el;
	...
	#fillBlcsMap( blc ) {
		if ( blc.classList.contains( "mzruiBlocksManager-block-selected" ) ) {
	...
	#getBeatSnap() {
		return 1 / this.timeline.stepsPerBeat * MZRUI.$getAttributeNum( this.timeline, "step" );
	}
	...
	onmousedown( e ) {
		...
		if ( e.button === 2 ) {
			this.#mmFn = mzruiBlocksManager.#mousemoveFns.deletion;
			this.#status = "deleting";
			if ( blc ) {
			...
			if ( e.shiftKey ) {
				this.#mmFn = mzruiBlocksManager.#mousemoveFns.selection1;
				this.#status = "selecting-1";
				this.#mdRowInd = this.getRowIndexByPageY( e.pageY );
			} else if ( blc ) {
				const fnAct = mzruiBlocksManager.#mousedownFns[ e.target.dataset.action ];

				if ( fnAct ) {
					const blcsEditing = this.#fillBlcsMap( blc );

					blc.classList.add( "mzrui-hover" );
					e.target.classList.add( "mzrui-hover" );
	...
	static #onmousedownMove( data, blcsEditing, _blc, e ) {
		this.#mmFn = mzruiBlocksManager.#mousemoveFns.move;
		this.#status = "moving";
		this.#mdRowInd = this.getRowIndexByPageY( e.pageY );
	...
	static #onmousedownCropA( data, blcsEditing ) {
		this.#mmFn = mzruiBlocksManager.#mousemoveFns.crop;
	...
	static #onmousemoveSelection1() {
		if ( Math.abs( this.#mmPageX - this.#mdPageX ) > 6 ||
			...
			this.#elSelection.classList.remove( "mzruiBlocksManager-selection-hidden" );
			this.#mmFn = mzruiBlocksManager.#mousemoveFns.selection2;
			this.#mmFn();
		}
	}
	...
	#onmouseup() {
		if ( this.#status ) {
			mzruiBlocksManager.#mouseupFns[ this.#status ].call( this, this.#blcsEditing, this.#mdBlc );
		}
		if ( this.#mdBlc ) {
			this.#mdBlc.classList.remove( "mzrui-hover" );
			this.#mdTarget.classList.remove( "mzrui-hover" );
		}
		this.#status = "";
		this.#mmFn =
		this.#mdBlc =
	...
	static #onmouseupSelecting1( blcsEditing, mdBlc ) {
		if ( mdBlc ) {
			mdBlc.classList.contains( "mzruiBlocksManager-block-selected" )
	...
	static #onmouseupSelecting2( blcsEditing ) {
		this.#elSelection.classList.add( "mzruiBlocksManager-selection-hidden" );
		if ( blcsEditing.size ) {
...
Object.freeze( mzruiBlocksManager );
