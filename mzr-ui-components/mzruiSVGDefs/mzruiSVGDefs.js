...
class mzruiSVGDefs {
	static #id = 0;
	#idPref = `mzruiSVGDefs_${ mzruiSVGDefs.#id++ }_`;
	#elDefs = MZRUI.$createElementSVG( "defs" );
	#defs = new Map();
	#w = 1;
	#h = 1;

	constructor() {
		const svg = MZRUI.$createElementSVG( "svg" );

		this.rootElement = svg;
		Object.freeze( this );

		svg.style.display = "none";
		svg.classList.add( "mzruiSVGDefs" );
	...
	add( id, w = 0, h = 0, ...elems ) {
		if ( this.#defs.has( id ) ) {
			console.error( `mzruiSVGDefs: ID "${ id }" already used` );
		} else {
			const g = MZRUI.$createElementSVG( "g" );
	...
	update( id, w, h, ...elems ) {
		...
		def.w = w;
		def.h = h;
		MZRUI.$emptyElement( def.g );
		def.g.append( ...elems );
	}
	createSVG( id ) {
		const def = this.#defs.get( id ) || {};
		const use = MZRUI.$createElementSVG( "use" );
		const svg = MZRUI.$createElementSVG( "svg", {
	...
	setSVGViewbox( svg, x, w ) {
		...
		MZRUI.$setAttribute( svg, "viewBox", `${ x } 0 ${ w } ${ h }` );
	}
}

Object.freeze( mzruiSVGDefs );
