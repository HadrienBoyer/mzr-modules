...
class mzruiPeriodicWave extends HTMLElement {
	...
	draw() {
		if ( this.firstChild && this.type ) {
			const wave = mzruiPeriodicWave.cache[ this.type ];

			if ( wave ) {
				this.#draw( wave );
			} else {
				console.error( `mzruiPeriodicWave: the wave "${ this.type }" is undefined...` );
			}
		}
	}
	#draw( wave ) {
		...
		MZRUI.$setAttribute( this.#svg.firstChild, "points", pts.join( " " ) );
	}
	...
	static addWave( name, real, imag ) {
		if ( !mzruiPeriodicWave.cache[ name ] ) {
			const arr = [];
			const fn = mzruiPeriodicWave.getXFromWave.bind( null, real, imag );

			for ( let x = 0; x < 256; ++x ) {
				arr.push( fn( x / 256 ) );
			}
			mzruiPeriodicWave.cache[ name ] = arr;
		}
	}
}

Object.freeze( mzruiPeriodicWave );
customElements.define( "mzrui-periodicwave", mzruiPeriodicWave );
