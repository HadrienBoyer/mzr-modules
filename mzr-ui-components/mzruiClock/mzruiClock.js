...
class mzruiClock extends HTMLElement {
	#bPM = 4;
	#sPB = 4;
	#timeSave = 0;
	#firstValueLen = -1;
	#attached = false;
	#values = [ -1, -1, -1 ];
	#children = MZRUI.$getTemplate( "mzrui-clock" );
	#elements = MZRUI.$findElements( this.#children, {
		wrapRel: ".mzruiClock-relative",
		modes: ".mzruiClock-modes",
		nodes: [
			".mzruiClock-a",
			".mzruiClock-b",
			".mzruiClock-c",
	...
	constructor() {
		super();
		this.onchangeDisplay = MZRUI.$noop;
		Object.seal( this );
	...
	static numbering( from ) {
		mzruiClock.numberingOff = +from;
		document.querySelectorAll( "mzrui-clock" ).forEach( el => el.resetTime() );
	}
	...
	static parseBeatsToBeats( beats, bPM, sPB ) {
		...
		return [
			`${ measures + mzruiClock.numberingOff }`,
			`${ steps + mzruiClock.numberingOff }`.padStart( 2, "0" ),
	...
	// .........................................................................
	setTime( beats ) {
		const [ a, b, c ] = MZRUI.$getAttribute( this, "mode" ) === "second"
			? mzruiClock.parseBeatsToSeconds( beats, MZRUI.$getAttributeNum( this, "bpm" ) || 60 )
			: mzruiClock.parseBeatsToBeats( beats, this.#bPM, this.#sPB );
	...
	// .........................................................................
	#onclickModes() {
		const dpl = MZRUI.$getAttribute( this, "mode" ) === "second" ? "beat" : "second";

		MZRUI.$setAttribute( this, "mode", dpl );
		this.onchangeDisplay( dpl );
	}
}

Object.seal( mzruiClock );
customElements.define( "mzrui-clock", mzruiClock );
