...
class mzruiKeys extends HTMLElement {
	...
	octaves( start, nbOct ) {
		...
		for ( let i = 0; i < nbOct; ++i ) {
			this.append( ...MZRUI.$getTemplate( "mzrui-keys-octave" ) );
		}
	...
	#keyUpDown( elKey, status ) {
		...
		elKey.classList.toggle( "mzrui-active", status );
		if ( status ) {
			this.#keysDown.set( midi );
			MZRUI.$dispatchEvent( this, "mzruiKeys", "keyDown", midi, this.#gain );
		} else {
			this.#keysDown.delete( midi );
			MZRUI.$dispatchEvent( this, "mzruiKeys", "keyUp", midi, this.#gain );
		}
	}
...
Object.freeze( mzruiKeys );
customElements.define( "mzrui-keys", mzruiKeys );
