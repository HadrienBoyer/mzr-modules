...
class mzruiSliderGroup extends HTMLElement {
	#min = 0;
	#max = 0;
	#def = 0;
	#step = 0;
	...
	#children = MZRUI.$getTemplate( "mzrui-slidergroup" );
	#elements = MZRUI.$findElements( this.#children, {
		slidersParent: ".mzruiSliderGroup-sliders",
		defValue: ".mzruiSliderGroup-defaultValue",
		beatlines: "mzrui-beatlines",
		currentTime: ".mzruiSliderGroup-currentTime",
		loopA: ".mzruiSliderGroup-loopA",
		loopB: ".mzruiSliderGroup-loopB",
	} );
	...
	connectedCallback() {
		if ( !this.firstChild ) {
			...
			MZRUI.$recallAttributes( this, { pxperbeat: 64 } );
			if ( beatlines ) {
				MZRUI.$recallAttributes( this, { currenttime: 0, timedivision: "4/4" } );
			}
		}
	}
		...
		if ( prev !== val ) {
			switch ( prop ) {
				...
				case "currenttime":
					...
					break;
				case "loopa":
					this.#elements.loopA.classList.toggle( "mzruiSliderGroup-loopOn", val );
					if ( val ) {
						this.#elements.loopA.style.width = `${ val }em`;
					}
					break;
				case "loopb":
					this.#elements.loopB.classList.toggle( "mzruiSliderGroup-loopOn", val );
					if ( val ) {
	...
	set( id, when, duration, value ) {
		const element = MZRUI.$getTemplate( "mzrui-slidergroup-slider" );
	...
	#updatePxPerBeat() {
		const ppb = MZRUI.$getAttributeNum( this, "pxperbeat" );

		this.#elements.slidersParent.style.fontSize = `${ ppb }px`;
		if ( this.#elements.beatlines ) {
			MZRUI.$setAttribute( this.#elements.beatlines, "pxperbeat", ppb );
		}
	}
	#sliderWhen( sli, when ) {
	...
	#sliderSelected( sli, b ) {
		if ( b !== this.#selected.has( sli.element.dataset.id ) ) {
			...
			sli.element.classList.toggle( "mzruiSliderGroup-sliderSelected", !!b );
	...
	#sliderSelectedClass() {
		this.#elements.slidersParent.classList.toggle(
			"mzruiSliderGroup-slidersSelected", this.#selected.size > 0 );
	}
	#sliderValue( sli, val ) {
		...
		st.height = `${ percX }%`;
		if ( el.classList.toggle( "mzruiSliderGroup-sliderInnerDown", valNum < 0 ) ) {
			st.top = `${ 100 - perc0 }%`;
			st.bottom = "auto";
		} else {
			st.top = "auto";
	...
	#mousemove( e ) {
		...
		const min = this.#min;
		const max = this.#max;
		const xval = x / MZRUI.$getAttributeNum( this, "pxperbeat" );
		...
		sliders.forEach( sli => {
			if ( firstWhen <= sli.when && sli.when <= xval && xval <= sli.when + sli.duration ) {
				...
				MZRUI.$dispatchEvent( this, "mzruiSliderGroup", "input", sli.element.dataset.id, rval );
			}
		} );
	}
	#mouseup() {
		...
		if ( arr.length ) {
			MZRUI.$dispatchEvent( this, "mzruiSliderGroup", "change", arr );
		}
		MZRUI.$dispatchEvent( this, "mzruiSliderGroup", "inputEnd" );
	}
}

Object.freeze( mzruiSliderGroup );
customElements.define( "mzrui-slidergroup", mzruiSliderGroup );
