...
class mzruiDotline {
	constructor() {
		const polyline = MZRUI.$createElementSVG( "polyline" );
		const svg = MZRUI.$createElementSVG( "svg", null, polyline );
		const root = MZRUI.$createElement( "div", { class: "mzruiDotline" }, svg );
		...
		this._dotMinY =
		this._mousebtn = 0;
		this._nlDots = root.getElementsByClassName( "mzruiDotline-dot" );
		this._rootBCR =
		this._activeDot =
	...
	resize() {
		...
		this._svgW = w;
		this._svgH = h;
		MZRUI.$setAttribute( this._elSVG, "viewBox", `0 0 ${ w } ${ h }` );
		this._drawPolyline();
	}
	options( obj ) {
		const opt = this._opt;
	...
	_drawPolyline() {
		...
		MZRUI.$setAttribute( this._elPoly, "points", arr.join( " " ) );
	}
	_onchange() {
		const obj = {};
	...
	_createDotElement( id ) {
		const el = MZRUI.$createElement( "div", { class: "mzruiDotline-dot", "data-id": id } );
	...
	_selectDotElement( id, b ) {
		...
		dot.element.classList.toggle( "mzruiDotline-dotSelected", b );
	}
