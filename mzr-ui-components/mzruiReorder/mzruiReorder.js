...
class mzruiReorder {
	constructor( opt = {} ) {
		...
		this._onchange = opt.onchange ?? ( () => console.warn( "mzruiReorder: no onchange set" ) );
	...
	_ondragstart( e ) {
		if ( this._elClicked && this._elClicked.matches( this._handleSelector ) ) {
			...
			setTimeout( () => {
				this._elDragged.classList.add( "mzruiReorder-dragging" );
				if ( this._elShadowDragged ) {
					this._elShadowDragged.classList.add( "mzruiReorder-dragging" );
				}
			}, 20 );
	...
	_ondragend() {
		if ( this._elDragged ) {
			...
			el.classList.remove( "mzruiReorder-dragging" );
			if ( this._elShadowDragged ) {
				this._elShadowDragged.classList.remove( "mzruiReorder-dragging" );
			}
