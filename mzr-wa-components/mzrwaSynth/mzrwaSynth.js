...
class mzrwaSynth {
	...
	startKey( blocks, when, off, dur ) {
		const id = ++mzrwaSynth.#startedMaxId;
		...
		const key = Object.freeze( {
			...
			variations: [],
			oscNodes: new Map(),
			gainEnvNode: new mzrwaEnvelope( ctx ),
			gainLFO: new mzrwaLFO( ctx, gainLFOtarget.gain ),
			gainLFOtarget,
	...
	stopKey( id ) {
		...
		if ( key ) {
			...
			console.error( "mzrwaSynth: stopKey id invalid", id );
		}
	}
	#stopKey( id ) {
		const key = this.#startedKeys.get( id );
	...
	#scheduleVariations( key ) {
		key.variations.forEach( va => {
			...
			const freqArr = new Float32Array( [
				mzrwaSynth.getHz( va.midi[ 0 ] ),
				mzrwaSynth.getHz( va.midi[ 1 ] )
			] );
	...
	#createOscNode( key, id ) {
		...
		oscNode.frequency.setValueAtTime( mzrwaSynth.getHz( key.midi ), atTime );
	...
	#nodeOscSetType( oscNode, type ) {
		if ( mzrwaSynth.nativeTypes.indexOf( type ) > -1 ) {
			oscNode.type = type;
		} else {
			oscNode.setPeriodicWave( mzrwaPeriodicWaves.get( this.ctx, type ) );
		}
	}
}

Object.freeze( mzrwaSynth );
