...
class mzrwaMixer {
	...
	ctx = null;
	connectedTo = null;
	audioDataL = new Uint8Array( mzrwaMixer.fftSize / 2 );
	audioDataR = new Uint8Array( mzrwaMixer.fftSize / 2 );
	#chans = {};
	...
	#addChan( id ) {
		const ctx = this.ctx;
		const pan = new mzrwaStereoPanner( ctx );
		...
		analyserL.fftSize =
		analyserR.fftSize = mzrwaMixer.fftSize;
