...
class mzrwaEncodeWAV {
	static encode( buffer, opt ) {
		...
		const samples = nbChannels === 2
			? mzrwaEncodeWAV.#interleave( buffer.getChannelData( 0 ), buffer.getChannelData( 1 ) )
			: buffer.getChannelData( 0 );
		const dataSize = samples.length * bytesPerSample;
		const arrBuffer = new ArrayBuffer( 44 + dataSize );
		const data = new DataView( arrBuffer );

		mzrwaEncodeWAV.#setString( data, 0, "RIFF" );           // FileTypeBlocID(4) : "RIFF"
		data.setUint32( 4, 36 + dataSize, true );              // FileSize(4)       : headerSize + dataSize - 8
		mzrwaEncodeWAV.#setString( data, 8, "WAVE" );           // FileFormatID(4)   : "WAVE"
		mzrwaEncodeWAV.#setString( data, 12, "fmt " );          // FormatBlocID(4)   : "fmt "
		...
		mzrwaEncodeWAV.#setString( data, 36, "data" );          // DataBlocID(4)     : "data"
		data.setUint32( 40, dataSize, true );                  // dataSize(4)       : fileSize - 44
		( format === 1
			? mzrwaEncodeWAV.#bufToInt16
			: mzrwaEncodeWAV.#bufToFloat32
		)( data, 44, samples );
		return arrBuffer;
	}
